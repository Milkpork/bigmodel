...
from deepspeed.utils.constants import ReduceOp
...
class TorchBackend(Backend):
    ...
    def reduce_scatter(self, output, input_list, op=ReduceOp.SUM, group=None, async_op=False):
        if DS_COMM_REDUCE_SCATTER_OFF:
            if int(os.getenv('RANK', '0')) == 0:
                logger.warning("REDUCE SCATTER  is OFF")
            return Noop()
        else:
    ...
    def broadcast(self, tensor, src, group=None, async_op=False):
        if DS_COMM_BROADCAST_OFF:
            if int(os.getenv('RANK', '0')) == 0:
                logger.warning("BROADCAST  is OFF")
            return Noop()
        else:
    ...
    def all_gather(self, tensor_list, tensor, group=None, async_op=False):
        if DS_COMM_ALL_GATHER_OFF:
            if int(os.getenv('RANK', '0')) == 0:
                logger.warning("All Gather is OFF")
            return Noop()
        else:
    ...
    def all_gather_base(self, output_tensor, input_tensor, group=None, async_op=False):
        if DS_COMM_ALL_GATHER_OFF:
            ...
            if self.has_allgather_base:
                ...
                logger.warning("unable to find torch.distributed._all_gather_base. will fall back to "
    ...
    def reduce_scatter_tensor(self, output_tensor, input_tensor, op=ReduceOp.SUM, group=None, async_op=False):
        if self.has_reduce_scatter_tensor():
            ...
            logger.warning("unable to find torch.distributed.reduce_scatter_tensor. will fall back to "
                           "torch.distributed.reduce_scatter which will result in suboptimal performance. "
                           "please consider upgrading your pytorch installation.")
            pass
